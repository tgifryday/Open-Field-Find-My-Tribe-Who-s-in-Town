    ...
    <div className="pb-28 pt-16">
      <div className="px-5 py-5 space-y-6">
        ...
        <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-6 text-white">
          ...
          <h2 className="text-2xl font-bold mb-1">{user.name === 'You' ? 'Explorer' : user.name}</h2>
          ...
          <div className="flex gap-3 mt-5">
            <div className="bg-white/20 rounded-xl px-4 py-3 text-center flex-1">
              <p className="text-xl font-bold">{nearbyUsers.length}</p>
              <p className="text-xs text-emerald-100">Nearby</p>
            </div>
            <div className="bg-white/20 rounded-xl px-4 py-3 text-center flex-1">
              <p className="text-xl font-bold">{userTribes.length}</p>
              <p className="text-xs text-emerald-100">Tribes</p>
            </div>
            <div className="bg-white/20 rounded-xl px-4 py-3 text-center flex-1">
              <p className="text-xl font-bold">{incomingCallouts.length}</p>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-slate-600 mt-1.5 truncate">{callout.message}</p>
            ...
            {nearbyUsers.slice(0, 6).map((person) => (
              <div key={person.id} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 min-w-[150px] shrink-0">
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{person.distance} mi away</p>
                  <div className="flex flex-wrap gap-1 mt-2.5 justify-center">
                    {person.interests.slice(0, 2).map((i) => (
                      <span key={i} className="text-xs bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-full">{i}</span>
                    ))}
                  </div>
                </div>
              </div>
